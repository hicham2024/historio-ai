...
function scoreResult(item, keyword) {
  ...
  if (item.source === "gallica") score += 10;
...
function archivePdfUrl(identifier, files = []) {
  const pdf = files.find((file) => file.name && file.name.toLowerCase().endsWith(".pdf"));
...
async function searchArchive(keyword) {
  try {
    ...
    ["title", "creator", "date", "identifier", "language", "description"].forEach((field) => {
    ...
    return await Promise.all(
      docs.map(async (doc) => {
        ...
        return {
          ...
          author: Array.isArray(doc.creator) ? doc.creator.join(", ") : doc.creator || "Auteur non indiqué",
          date: doc.date || "Date non indiquée",
          language: Array.isArray(doc.language) ? doc.language.join(", ") : doc.language || "Langue non indiquée",
          description: stripHtml(Array.isArray(doc.description) ? doc.description[0] : doc.description || ""),
    ...
    return (data?.items || [])
      ...
      .map((item) => {
        ...
        return {
          ...
          author: Array.isArray(info.authors) ? info.authors.join(", ") : "Auteur non indiqué",
          ...
          access: access.pdf?.downloadLink ? "PDF Google Books" : "Lecture gratuite",
        };
      });
  } catch {
    return [];
...
function parseGallicaXml(xmlText) {
  ...
  return records.map((record, index) => {
    const get = (tag) => record.getElementsByTagName(tag)?.[0]?.textContent || "";
    const title = get("dc:title") || "Titre non indiqué";
    const identifierNodes = Array.from(record.getElementsByTagName("dc:identifier"));
    const pageUrl = identifierNodes
      .map((node) => node.textContent || "")
      .find((value) => value.includes("gallica.bnf.fr")) || "";
    const arkMatch = pageUrl.match(/ark:\/\/[0-9]+\/[^/?#]+/);
    const directPdf = arkMatch ? `https://gallica.bnf.fr/${arkMatch[0]}.pdf` : "";
    ...
    return {
      ...
      pageUrl,
      directPdf,
      access: directPdf ? "PDF Gallica" : "Page Gallica",
    };
  });
}
  ...
  return (
    <article className="result-item">
      <div>
        ...
        <div className="meta">{item.author} — {item.date} — {item.language}</div>
        {item.description && (
...
export default function App() {
  ...
  const [email, setEmail] = useState(localStorage.getItem("historio_user_email") || "");
  ...
  function connectWithEmail(event) {
    event.preventDefault();
    const value = emailInput.trim().toLowerCase();
    if (!/^\S+@\S+\.\S+$/.test(value)) return;
    localStorage.setItem("historio_user_email", value);
    setEmail(value);
  }

  function logout() {
    localStorage.removeItem("historio_user_email");
    setEmail("");
    setResults([]);
  }
  ...
  async function runSearch(event) {
    ...
    const merged = dedupeResults(settled.flat()).sort((a, b) => scoreResult(b, clean) - scoreResult(a, clean));
    setResults(merged);
    setLoading(false);
  }
    ...
    <div className="page">
      <style>{`
        * { box-sizing: border-box; }
        ...
        .donation-panel { max-width: 820px; margin-top: 14px; background: #f0fdfa; border: 1px solid #99f6e4; color: #134e4a; padding: 14px; }
        .donation-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-top: 10px; }
        .donation-card { background: white; border: 1px solid #ccfbf1; padding: 12px; min-height: 94px; font-size: 13px; }
        .donation-card strong { display: block; color: #7f1d1d; margin-bottom: 6px; }
        .qr { height: 50px; border: 1px dashed #0f766e; margin-top: 8px; display: flex; align-items: center; justify-content: center; color: #0f766e; }
        ...
        .content-header { border-bottom: 1px solid #e2d8c8; padding-bottom: 12px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; }
      ...
      <header className="topbar">
        <div className="top-inner">
          <div className="brand"><BookOpen size={32} /><strong>Historio</strong><span>PDF</span></div>
          <div className="subtitle">Moteur de recherche de livres historiques gratuits, PDF et documents publics.</div>

          <div className="userbar">
            <div>{email ? <>Connecté : <strong>{email}</strong></> : <>Connectez-vous avec votre e-mail pour utiliser la recherche.</>}</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button type="button" onClick={() => setShowDonate(!showDonate)}><Heart size={14} /> Faire un don</button>
              {email && <button type="button" onClick={logout}><LogOut size={14} /> Déconnexion</button>}
            </div>
          </div>

          {!email && (
            <div className="gate">
              <div className="gate-title"><Lock size={18} /> Accès par e-mail requis</div>
              <div>Entrez votre e-mail pour accéder à la recherche. Cette version stocke l’e-mail localement dans votre navigateur.</div>
              <form onSubmit={connectWithEmail}>
                <input value={emailInput} onChange={(e) => setEmailInput(e.target.value)} placeholder="votre@email.com" type="email" />
                <button type="submit"><Mail size={16} /> Se connecter</button>
              </form>
            </div>
          )}
          ...
          {showDonate && (
            <div className="donation-panel">
              <strong>Soutenir HistorioPDF</strong>
              <div>Remplacez ces exemples par vos vrais liens PayPal, Mollie, Stripe ou Bancontact.</div>
              <div className="donation-grid">
                <div className="donation-card"><strong>Virement bancaire</strong><div>IBAN : BE00 0000 0000 0000</div><div className="qr"><QrCode size={26} /></div></div>
                <div className="donation-card"><strong>PayPal</strong><div>paypal.me/votrepage</div><div className="qr"><QrCode size={26} /></div></div>
                <div className="donation-card"><strong>Visa / carte</strong><div>Lien Stripe ou Mollie</div><div className="qr"><CreditCard size={26} /></div></div>
                <div className="donation-card"><strong>Bancontact</strong><div>QR Mollie / Payconiq</div><div className="qr"><QrCode size={26} /></div></div>
              </div>
            </div>
          )}

          <form className={`search-row ${!email ? "disabled" : ""}`} onSubmit={runSearch}>
            <input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="Rechercher un livre historique, un PDF, une archive…" disabled={!email} />
            <button disabled={loading || !email} type="submit">{loading ? <Loader2 className="spin" size={22} /> : <Search size={22} />}</button>
          </form>
        </div>
      </header>
      ...
      <main className="layout">
        <aside className="sidebar">
          <div className="filter-block">
            ...
            <button className={`filter-link ${sourceFilter === "all" ? "active" : ""}`} onClick={() => setSourceFilter("all")}>Toutes les sources</button>
            <button className={`filter-link ${sourceFilter === "archive" ? "active" : ""}`} onClick={() => setSourceFilter("archive")}>Archive.org</button>
            <button className={`filter-link ${sourceFilter === "googleBooks" ? "active" : ""}`} onClick={() => setSourceFilter("googleBooks")}>Google Books gratuit</button>
            <button className={`filter-link ${sourceFilter === "gallica" ? "active" : ""}`} onClick={() => setSourceFilter("gallica")}>Gallica / BnF</button>
          </div>
          <div className="filter-block">
            <div className="filter-title">Accès</div>
            <label className="check"><input type="checkbox" checked={onlyPdf} onChange={(e) => setOnlyPdf(e.target.checked)} /> PDF direct uniquement</label>
          </div>
        </aside>
        ...
        <section>
          ...
          {loading && <div className="loading"><Loader2 className="spin" size={20} /> Recherche en cours…</div>}
          {!loading && filtered.length === 0 && <div className="empty">{email ? "Aucun PDF trouvé. Essayez un autre mot-clé ou désactivez “PDF direct uniquement”." : "Connectez-vous avec votre e-mail pour lancer une recherche."}</div>}

          <div className="results">
            {filtered.map((item) => <ResultItem key={item.id} item={item} />)}
          </div>
        </section>
      </main>
    </div>
